 ...
 * we goot and which is stored un usmb2->buff.
                       ...
                       char *password)
{
        ...
        ntlmssp_in_offset = le16toh(*(uint16_t *)(usmb2->buff + 4)) - 64;
        ...
        domain_name_len = le16toh(*(uint16_t *)(usmb2->buff + offset));
        domain_name = usmb2->buff + ntlmssp_in_offset + le32toh(*(uint32_t *)(usmb2->buff + offset + 4));
        ...
        server_challenge = usmb2->buff + ntlmssp_in_offset + 24;
        memcpy(usmb2->buff + 4 + 64 + 24 + ntlm_response_offset + 8, server_challenge, 8);
         ...
         */
        target_info_len = le16toh(*(uint16_t *)(usmb2->buff + ntlmssp_in_offset + 40));
        target_info = usmb2->buff + ntlmssp_in_offset + le32toh(*(uint32_t *)(usmb2->buff + ntlmssp_in_offset + 44));
        ...
        memcpy(usmb2->buff + offset, target_info, z);
        
        /* Add TargetName Av entry to the end */
        offset += z;
        *(uint16_t *)(usmb2->buff + offset) = htole16(9);
        *(uint16_t *)(usmb2->buff + offset + 2) = htole16(10 + domain_name_len);
        offset += 4;
        *(uint16_t *)(usmb2->buff + offset) = htole16('c');
        offset += 2;
        *(uint16_t *)(usmb2->buff + offset) = htole16('i');
        offset += 2;
        *(uint16_t *)(usmb2->buff + offset) = htole16('f');
        offset += 2;
        *(uint16_t *)(usmb2->buff + offset) = htole16('s');
        offset += 2;
        *(uint16_t *)(usmb2->buff + offset) = htole16('/');
        offset += 2;
        memcpy(usmb2->buff + offset, domain_name, domain_name_len);
        offset += domain_name_len;
        
        /* and a final EndOfList plus 4 bytes of padding*/
        memset(usmb2->buff + offset, 0, 8);
        offset += 8;
        out_pdu_size = offset;
         ...
         */
        memcpy(usmb2->buff + 4 + 64 + 24 + domain_name_offset, domain_name, domain_name_len);
        domain_name = usmb2->buff + 4 + 64 + 24 + domain_name_offset;
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 28) = htole32(domain_name_len << 16 | domain_name_len);
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 32) = htole32(domain_name_offset);

        /*
         * NT Response
         */
        at_type = out_pdu_size - 4 - 64 - 24 - ntlm_response_offset;
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 20) = htole32(at_type << 16 | at_type);
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 24) = htole32(ntlm_response_offset);
          
        /*
         * User name
         */
        for(at_type = 0; at_type < user_name_len;) {
                *(usmb2->buff + 4 + 64 + 24 + user_name_offset + at_type++) = username[at_type >> 1];
                *(usmb2->buff + 4 + 64 + 24 + user_name_offset + at_type++) = 0;
        }
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 36) = htole32(user_name_len << 16 | user_name_len);
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 40) = htole32(user_name_offset);
        /*
         * Host name and remaining fields
         */
        memset(usmb2->buff + ntlmssp_out_offset + 44, 0, 12);

        /* Negotiate flags */
        *(uint32_t *)(usmb2->buff + ntlmssp_out_offset + 56) = htole32(0x20088037);

        /* Version */
        memset(usmb2->buff + ntlmssp_out_offset + 60, 0, 8);

        memcpy(usmb2->buff + 4 + 64 + 24, "NTLMSSP", 8);
        memset(usmb2->buff + 4 + 64 + 24 + 8, 0, 12);
        *(usmb2->buff + 4 + 64 + 24 + 8) = 3;
         ...
         */
        memset(usmb2->buff + 4 + 64 + 24 + ntlm_response_offset + 16, 0, 28);
        /* resp type and hi resp type */
        *(uint16_t *)(usmb2->buff + 4 + 64 + 24 + ntlm_response_offset + 16) = 0x0101;
        /* client challenge QQQQ */
        memset(usmb2->buff + 4 + 64 + 24 + ntlm_response_offset + 32, 1, 8);
        ...
        hmac_md5(NULL,
                 usmb2->buff + at_type + 8,  at_len - 8,
                 NULL, 0,
                 NTOWFv2, 16, NTProofStr);
        memcpy(usmb2->buff + 4 + 64 + 24 + ntlm_response_offset, NTProofStr, 16);
